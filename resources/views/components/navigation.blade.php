<nav class="flex-1 px-3 py-6 space-y-1 overflow-y-auto">
    ...
    @if($isAdmin)
        ...
        <a href="{{ route('admin.assignments.requests.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-lg {{ request()->routeIs('admin.assignments.requests.*') ? 'bg-indigo-500/20 text-indigo-400' : 'text-slate-400 hover:bg-slate-800/50 hover:text-white' }} transition-all duration-200">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z"></path>
            </svg>
            <span class="font-medium">Analyst Requests</span>
        </a>
        ...
        <a href="{{ route('notifications.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-lg {{ request()->routeIs('notifications.*') ? 'bg-yellow-500/20 text-yellow-400' : 'text-slate-400 hover:bg-slate-800/50 hover:text-white' }} transition-all duration-200 relative">
            ...
            @endphp
            @if($unreadCount > 0)
                <span class="ml-auto min-w-[20px] h-5 px-1.5 bg-red-500 text-white text-xs font-bold rounded-full flex items-center justify-center shadow-sm" style="background-color: #ef4444; color: white;">
    ...
    @if($isTrader)
        ...
        <a href="{{ route('trader.analyst-request.create') }}" class="flex items-center gap-3 px-4 py-3 rounded-lg {{ request()->routeIs('trader.analyst-request.*') ? 'bg-indigo-500/20 text-indigo-400' : 'text-slate-400 hover:bg-slate-800/50 hover:text-white' }} transition-all duration-200">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0"></path>
            </svg>
            <span class="font-medium">Performance Analyst</span>
        </a>

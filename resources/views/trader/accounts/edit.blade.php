...
<div class="max-w-2xl mx-auto">
    ...
    <nav class="flex mb-8 text-sm text-slate-400">
        ...
        <a href="{{ route('trader.accounts.show', $account) }}" class="hover:text-white transition-colors">{{ $account->account_name }}</a>
        <span class="mx-2">/</span>
    ...
    <!-- Header -->
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-white mb-2">Edit Account</h1>
        <p class="text-slate-400">Update account details for {{ $account->account_name }}</p>
    </div>

    <!-- Form -->
    <div class="bg-slate-800/50 backdrop-blur-xl rounded-xl p-8 border border-slate-700/50 shadow-xl">
        <form action="{{ route('trader.accounts.update', $account) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <div class="space-y-3">
                        ...
                        <label class="flex items-center p-3 rounded-lg border border-slate-700 cursor-pointer hover:bg-slate-700/50 transition-colors {{ old('account_type', $account->account_type->value) == $type->value ? 'bg-blue-500/10 border-blue-500' : 'bg-slate-900/30' }}">
                            <input type="radio" name="account_type" value="{{ $type->value }}" 
                                ...
                                {{ old('account_type', $account->account_type->value) == $type->value ? 'checked' : '' }} required>
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="broker" id="broker" 
                            ...
                            value="{{ old('broker', $account->broker) }}">
                    </div>
                    ...
                    <!-- Initial Balance (Disabled - Use Transactions instead) -->
                    <div>
                        <label class="block text-sm font-medium text-slate-500 mb-2">Initial Balance (Read Only)</label>
                        <div class="relative opacity-50 cursor-not-allowed">
                            ...
                            <input type="text" value="{{ $account->initial_balance }}" disabled
                                class="w-full bg-slate-800 border border-slate-700 rounded-lg pl-8 pr-4 py-3 text-slate-400">
                        </div>
                        <p class="text-xs text-slate-500 mt-1">To change balance, add a deposit/withdrawal transaction.</p>
                    </div>
                </div>
            </div>

            <!-- Submit Buttons -->
            <div class="flex items-center justify-between gap-4 mt-8 pt-6 border-t border-slate-700/50">
                <button type="button" onclick="confirmDelete()" class="text-red-400 hover:text-red-300 text-sm font-medium transition-colors">
                    Delete Account
                </button>

                <div class="flex items-center gap-4">
                    <a href="{{ route('trader.accounts.show', $account) }}" class="px-6 py-2.5 text-slate-300 hover:text-white font-medium transition-colors">Cancel</a>
        ...
        <form id="delete-form" action="{{ route('trader.accounts.destroy', $account) }}" method="POST" class="hidden">
            @csrf
            @method('DELETE')
        </form>
    </div>

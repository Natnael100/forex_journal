...
@section('content')
    ...
    <div class="mb-8">
        <div class="flex items-center gap-4 mb-4">
            <a href="{{ route('trader.trades.show', $trade) }}" class="text-slate-400 hover:text-white transition-colors">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path>
                </svg>
            </a>
            <div>
                <h1 class="text-3xl font-bold text-white">Edit Trade</h1>
                <p class="text-slate-400">Update your trading record</p>
            </div>
        </div>
    </div>

    <!-- Form -->
    <form method="POST" action="{{ route('trader.trades.update', $trade) }}" class="max-w-4xl">
        @csrf
        @method('PUT')
        ...
        <div class="bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-xl rounded-xl p-8 border border-slate-700/50 space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        >
                            ...
                            @foreach($pairs as $pair)
                                <option value="{{ $pair }}" class="bg-slate-800" {{ old('pair', $trade->pair) == $pair ? 'selected' : '' }}>{{ $pair }}</option>
                            @endforeach
                        </select>
                        @error('pair')
                            ...
                            @foreach($directions as $direction)
                                <label class="flex-1 relative cursor-pointer">
                                    <input 
                                        ...
                                        required
                                        {{ old('direction', $trade->direction->value) == $direction->value ? 'checked' : '' }}
                                        class="peer sr-only"
                                    >
                    ...
                    <div>
                        ...
                        <input 
                            ...
                            id="exit_date" 
                            name="exit_date"
                            value="{{ old('exit_date', $trade->exit_date?->format('Y-m-d\TH:i')) }}"
                    ...
                    <div>
                        ...
                        >
                            ...
                            @foreach($sessions as $session)
                                <option value="{{ $session->value }}" class="bg-slate-800" {{ old('session', $trade->session->value) == $session->value ? 'selected' : '' }}>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input 
                            ...
                            id="emotion" 
                            name="emotion"
                            value="{{ old('emotion', $trade->emotion) }}"
                    ...
                    <div>
                        ...
                        <input 
                            ...
                            step="0.01"
                            min="0"
                            value="{{ old('risk_reward_ratio', $trade->risk_reward_ratio) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            @foreach($outcomes as $outcome)
                                <label class="relative cursor-pointer">
                                    <input 
                                        ...
                                        value="{{ $outcome->value }}" 
                                        required
                                        {{ old('outcome', $trade->outcome->value) == $outcome->value ? 'checked' : '' }}
                                        class="peer sr-only"
                                    >
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input 
                            ...
                            step="0.01"
                            required
                            value="{{ old('profit_loss', $trade->profit_loss) }}"
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input 
                            ...
                            value="{{ old('tradingview_link', $trade->tradingview_link) }}"
                    ...
                    <div>
                        ...
                        >{{ old('notes', $trade->notes) }}</textarea>
                        @error('notes')
                            <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="flex gap-4 pt-6 border-t border-slate-700/50">
                ...
                <a 
                    href="{{ route('trader.trades.show', $trade) }}" 
